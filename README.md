...
- 智能選重:
    ...
    - 例如 "休息" 一詞, 編碼爲 `efkmgk`, 其中 `efk`/`mgk` 的首選分别是 "条"/"息", 爲了打出 "休息", 引入該六碼詞.
...
[^1]: 關於四碼詞語，在吉旦餅的發版中是没有預設的，用户如有需要，請自行在smyh.yuhaowords.dict.yaml中添加或import 詞典文件. 可不設編碼添加詞組，部署時會自動編碼.編碼規則爲:①二字词"首字首根大碼 + 首根小碼 + 次字首根大碼 + 次字首根小碼", ②三字詞: "首字首根 + 次字首根 + 末字首根 + 末字次根", ③四字及以上詞: "首字首根 + 次字首根 + 第三字首根 + 末字首根"
